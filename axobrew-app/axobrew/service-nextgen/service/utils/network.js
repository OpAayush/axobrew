    ...
    const attempt = (left) => {
        ...
        return fetchWithTimeout(url, timeoutMs)
            ...
            .catch(e => {
                ...
                if (left <= 0) throw e;
                return new Promise(resolve => setTimeout(resolve, left === 1 ? 3000 : 750))
                    .then(() => attempt(left - 1));
            });
    };
...
function fetchJsonWithRetry(url, timeoutMs, retries, label) {
    return fetchTextWithRetry(url, timeoutMs, retries, label)
        .then(text => {
            try {
...
module.exports = { fetchWithTimeout, fetchTextWithRetry, fetchJsonWithRetry };
