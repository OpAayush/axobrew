...
function Item({ children, module, id, state, shouldFocus }) {
  ...
  return (
    ...
    >
      {children}
      {module.dev && (
        <span className='absolute top-5 right-5 rounded-md bg-brew-amber px-2.5 py-1 text-[calc(var(--uh)*1.6)] font-bold tracking-widest text-ink-950'>
          DEV
        </span>
      )}
    </div>
  ...
  return (
    <Item module={module} id={id} state={state} shouldFocus={id === 0}>
      ...
      <div className='mt-4 flex items-center gap-3'>
        ...
        <span className='rounded-md bg-brew-amber/10 ring-1 ring-brew-amber/30 px-2 py-0.5 text-[calc(var(--uh)*1.7)] font-medium text-brew-amber tabular-nums'>
          v{module.version}
        </span>
      </div>
...
function SkeletonTile() {
  return (
    <div className={TILE_BASE}>
      <div className='skeleton h-[calc(var(--uh)*2.4)] w-1/4 rounded-md' />
      <div className='skeleton h-[calc(var(--uh)*3)] w-3/4 rounded-md mt-5' />
      <div className='skeleton h-[calc(var(--uh)*2)] w-full rounded-md mt-6' />
      <div className='skeleton h-[calc(var(--uh)*2)] w-4/5 rounded-md mt-2.5' />
    </div>
  );
}
    ...
    <div className="relative isolate lg:px-8">
      <div className="mx-auto flex flex-wrap justify-center gap-4 top-4 relative">
        {modules === null ? (
          ...
          <Tile extra='!h-[calc(var(--uh)*30)] !w-[42vw] flex flex-col items-center justify-center gap-5'>
            <ArchiveBoxIcon className='h-[calc(var(--uh)*7)] w-[calc(var(--uh)*7)] text-brew-cyan/60' />
            <h3 className='text-white text-[calc(var(--uh)*3)] font-semibold'>
              {t('modules.emptyTitle')}
            </h3>
            <p className='text-gray-400 text-[calc(var(--uh)*2)] leading-relaxed max-w-[26vw] text-center'>
              {t('modules.emptyDesc')}
            </p>
          </Tile>
        ) : (
